...
@section('content-layout')
    <div class="detail container  py-4">
        <div class="  picture--product ">
            <img src="{{ asset('source/images/room/' . $room->image) }}" alt="">
        </div>
        <div class=" information_product">
            <h3>{{ $room->name }}</h3>
            <h3></h3>
            <p class="discount--detail"> <span
                    style="text-decoration: line-through;">{{ number_format($room->price) }} VND</span> </p>
           
          
            <p>MÔ TẢ: <span> {{ $room->description }}</span>
            </p>
            <div id="buy-amount">
                ...
                <form action="{{ route('addToCart1', $room->id) }}" method="POST">
                    @csrf
